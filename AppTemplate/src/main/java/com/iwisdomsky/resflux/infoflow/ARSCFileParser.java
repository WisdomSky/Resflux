 ...
 * Bodden, and others.
 ******************************************************************************/
package com.iwisdomsky.resflux.infoflow;
...
public class ARSCFileParser extends AbstractResourceParser {
	...
	public class ColorResource extends AbstractResource {
		...
		public String getHexARGB() { return String.format("#%02x", this.a); }
	...
	private int readConfigTable
		...
		if (remainingSize > 0) {
			...
			if (!(new BigInteger(1, remainingBytes).equals(BigInteger.ZERO))) {
				Log.v("apk-parser", "Excessive non-null bytes in ResTable_Config ignored");
				assert false;
			}
